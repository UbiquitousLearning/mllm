...
def freeze_qwen2_rmsnorm_weight(m):
    if isinstance(m, QRMSNorm):
        m.freeze_weight()


def freeze_qwen2_linear_weight(m):
    if isinstance(m, QLinearLPBQ) or isinstance(m, QLinearW8A16_PerChannelSym):
        m.freeze_weight()


def freeze_qwen2_embed_tokens_weight(m):
...
class Qwen2Quantizer:
    def __init__(self, model_path: str, mllm_qualcomm_max_length=2048):
        ...
        # PTQ All Weights.
        self.model.apply(freeze_qwen2_rmsnorm_weight)
        self.model.apply(freeze_qwen2_linear_weight)
        self.model.apply(freeze_qwen2_embed_tokens_weight)
