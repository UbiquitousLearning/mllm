...
NetTensor *FFN(NetTensor *i, int hidden_dim, int ffn_hidden_dim, string name) {
    auto *x = _Linear({i}, hidden_dim, ffn_hidden_dim, false, name + ".fc1");
...
void opt(Context *c, int vocab_size = 32000, int hidden_dim = 4096, int ffn_hidden_dim = 11008, int mutil_head_size = 32, int cache_max = 200) {
    auto *i = _Input(c);
    // TODO： haven't implement embedding
    // i = _Embedding({i}, vocab_size, hidden_dim, (string) "embed_tokens");
    // loop
    for (int layer = 0; layer < 32; ++layer) {
        auto *x = *Attention(i, hidden_dim, hidden_dim / mutil_head_size, mutil_head_size, cache_max, (string) "model.decoder.layers." + std::to_string(layer) + ".self_attn") + i;
        x = _RMSNorm({i}, hidden_dim, 1e-6, (string) "model.decoder.layers." + std::to_string(layer) + ".self_attn_layer_norm");
        i = *FFN(x, hidden_dim, ffn_hidden_dim, (string) "model.decoder.layers." + std::to_string(layer)) + i;
        i = _RMSNorm({i}, hidden_dim, 1e-6, (string) "model.decoder.layers." + std::to_string(layer) + ".final_layer_norm");
    }
    // end loop
    i = _RMSNorm({i}, hidden_dim, 1e-6, (string) "model.decoder.final_layer_norm");
...
int main(int argc, char **argv) {
    ...
    cmdParser.add<string>("model", 'm', "specify mllm model path", false, "./models/opt-6.7B-smoothquant.mllm");
    ...
    auto *c = c_ptr.get();
    opt(c, vocab_size, hidden_dim, ffn_hidden_dim, mutil_head_size, tokens_limit);
