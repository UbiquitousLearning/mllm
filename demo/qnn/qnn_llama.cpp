...
void llama(Context *c, int vocab_size = 32000, int hidden_dim = 4096, int ffn_hidden_dim = 11008, int mutil_head_size = 32, int cache_max = 200) {
    ...
    for (int layer = 0; layer < 16; ++layer) {
        ...
        i = Attention(x, hidden_dim, hidden_dim / mutil_head_size, mutil_head_size, cache_max, (string) "model.layers." + std::to_string(layer) + ".self_attn");
        x = _RMSNorm({i}, hidden_dim, 1e-6, (string) "model.layers." + std::to_string(layer) + ".post_attention_layernorm");
        i = FFN(x, hidden_dim, ffn_hidden_dim, (string) "model.layers." + std::to_string(layer) + ".mlp");
        //_SubgraphBegin(c);
    }
    // end loop
...
int main(int argc, char **argv) {
    ...
    auto *c = c_ptr.get();
    llama(c, vocab_size, hidden_dim, ffn_hidden_dim, mutil_head_size, 1);
