...
NetTensor * RMSNorm(Context *ctx,  uint32_t hidden_dim, uint32_t ffn_hidden_dim) {

    auto *i = _Input(ctx);
    auto *z = _RMSNorm(ctx, {i}, "ffn.rmsnorm");
...
int main() {
    ...
    RMSNorm(c, hidden_dim, ffn_hidden_dim);
    ...
    textTensor(input, net, {1, 1, 1, vocab_size});
