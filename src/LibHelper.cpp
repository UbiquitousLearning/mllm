...
LibHelper::LibHelper(AAssetManager* asset_manager, std::string weights_path, std::string vacab_path) {
    asset_manager_=asset_manager;
    c = new Context();
    BackendConfig bn;
    const auto param_loader = new ParamLoader(std::move(weights_path),asset_manager_);
    executor_ =new  Executor(param_loader);
    net_ = new Net(bn);
    tokenizer_ = new BPETokenizer(std::move(vacab_path));
    tokenizer_->setAssetManager(asset_manager_);
} // mllm
void LibHelper::setUp(PreDefinedModel model, MLLMBackendType backend_type) {
    assert(model == LLAMA);
