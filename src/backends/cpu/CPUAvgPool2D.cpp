...
CPUAvgPool2D::CPUAvgPool2D(Backend *bn,  string opName, vector<int> kernal_size, vector<int> stride, PaddingType padding_type, bool multiThread) :
Op(bn, opName) {
...
ErrorCode CPUAvgPool2D::reshape(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
    //batch = batch
...
ErrorCode CPUAvgPool2D::execute(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
    ...
    switch (padding_type_) {
    case SAME:{
        avgpool2d_fp32_SAME(inputs[0].get(), outputs[0].get(),kernel_size_[0], kernel_size_[1], stride_[0], stride_[1], padding_h_, padding_w_);
        break;
    }
    case VALID: {
        avgpool2d_fp32_VALID(inputs[0].get(), outputs[0].get(), kernel_size_[0], kernel_size_[1], stride_[0], stride_[1]);
        break;
    }
    }
