...
CPUAvgPool2D::CPUAvgPool2D(Backend *bn,  string opName, vector<int> kernal_size, vector<int> stride, PaddingType padding_type, int threadCount) : thread_count(threadCount),
Op(bn, opName) {
...
ErrorCode CPUAvgPool2D::execute(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
    ...
    case VALID: {
        avgpool2d_fp32_VALID(inputs[0].get(), outputs[0].get(), kernel_size_[0], kernel_size_[1], stride_[0], stride_[1], thread_count);
        break;
    }
    }
