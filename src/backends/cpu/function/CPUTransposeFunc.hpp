...
public:
    ...
    void setup(vector<Tensor *> outputs, vector<Tensor *> inputs, vector<float> args) override {
        ...
        if (inputs[0]->masterTensor() != nullptr) {
            ...
            // auto a = std::equal(inputs[0]->chls().begin(), inputs[0]->chls().end(), origin_chls.begin());
            // if (std::equal(inputs[0]->chls().begin(), inputs[0]->chls().end(), origin_chls.begin())) {
            for (auto axis : axiss) {
                auto axis0 = axis.first;
                auto axis1 = axis.second;
                auto ori_0_idx = inputs[0]->chls()[axis0];
                auto ori_1_idx = inputs[0]->chls()[axis1];
                inputs[0]->chls()[axis0] = ori_1_idx;
                inputs[0]->chls()[axis1] = ori_0_idx;
            }
            inputs[0]->changeCtype(origin_s);
