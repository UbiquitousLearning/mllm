...
//    CHECK_EQ(inputs[0]->head(), 1);
    ...
    weight_.reshape(1, out_features_, in_features_, inputs[0]->head());
    weight_.setName(name() + ".weight");
    bias_.reshape(1, out_features_, 1, inputs[0]->head());
    bias_.setName(name() + ".bias");
    outputs[0]->reshape(inputs[0]->batch(), out_features_, inputs[0]->sequence(), inputs[0]->head());
    return NO_ERROR;
}
        ...
        for (int h = 0; h < inputs[0]->head(); h++) {
            for (int m = 0; m < M; m++) {
                for (int n = 0; n < N; n++) {
                    float value = 0;
                    for (int k = 0; k < K; k++) {
                        value += weight_.dataAt<float>(0, m, k, h) * inputs[0]->dataAt<float>(b, k, n, h);
                    }
                    if (support_bias_)
                        value += bias_.dataAt<float>(0, m, 0, h);
                    outputs[0]->setDataAt<float>(b, m, n, h, value);
                }
            }
        }
    }
