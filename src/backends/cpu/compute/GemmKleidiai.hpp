...
// #else
//     typedef uint16_t mllm_fp16_t; // Fallback for non-ARM compilation
// #endif
...
//#if defined(KLAI_USE_QSI4_C32)
// --- 实现 1: float * qsi4c32 -> float (底层API) ---

size_t mllm_kleidai_get_packed_b_qsi4_size(int N, int K);
size_t mllm_kleidai_get_workspace_qsi4_size(int M, int K);
...
//#if defined(KLAI_USE_FP32)
// --- 实现 3: float * fp32 -> float (底层API) ---
