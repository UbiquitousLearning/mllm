...
CPUHeadLinear::CPUHeadLinear(Backend *bn, string opName, int in_features, int out_features, bool bias, int threadCount) :
...
ErrorCode CPUHeadLinear::reshape(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
    // std::cout << name() << "  CPUHeadLinear  reshape" << std::endl;
...
ErrorCode CPUHeadLinear::load(AbstructLoader &loader) {
    // std::cout << name() << "  CPUHeadLinear load" << std::endl;
...
#pragma omp parallel for collapse(3) num_threads(thread_count)
        ...
        mat_mul(tmp_in.get(), &weight_, tmp_out.get(), support_bias_, &bias_, false, true, thread_count);
    }
    // std::cout << name() << "  CPUHeadLinear()" << std::endl;
    /*
...
ErrorCode CPUHeadLinear::free(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
    weight_.free();
    if (support_bias_) {
        bias_.free();
    }
