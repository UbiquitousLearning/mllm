...
#ifndef MLLM_CPUVISIONROPECOS_H
#define MLLM_CPUVISIONROPECOS_H
...
class CPUVisionRoPECos final : public Op {
public:
    CPUVisionRoPECos(Backend *bn, string opName, int dim, int spatial_merge_size, int threadCount);
    virtual ~CPUVisionRoPECos() = default;
...
class CPUVisionRoPECosCreator : public CPUBackend::Creator {
public:
    virtual Op *create(OpParam op_param, Backend *bn, string name, int threadCount) const {
        int dims = (int)op_param["dim"];
        int spatial_merge_size = (int)op_param["spatial_merge_size"];
        return new CPUVisionRoPECos(bn, name, dims, spatial_merge_size, threadCount);
    }
};
