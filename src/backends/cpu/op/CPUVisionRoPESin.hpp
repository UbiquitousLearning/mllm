...
#ifndef MLLM_CPUVISIONROPESIN_H
#define MLLM_CPUVISIONROPESIN_H
...
class CPUVisionRoPESin final : public Op {
public:
    CPUVisionRoPESin(Backend *bn, string opName, int dim, int spatial_merge_size, int threadCount);
    virtual ~CPUVisionRoPESin() = default;
...
class CPUVisionRoPESinCreator : public CPUBackend::Creator {
public:
    virtual Op *create(OpParam op_param, Backend *bn, string name, int threadCount) const {
        int dims = (int)op_param["dim"];
        int spatial_merge_size = (int)op_param["spatial_merge_size"];
        return new CPUVisionRoPESin(bn, name, dims, spatial_merge_size, threadCount);
    }
};
