...
namespace mllm {
CPULinearINT8Shadow::CPULinearINT8Shadow(Backend *bn, string opName, int in_features, int out_features, bool bias, int threadCount) : thread_count(threadCount),
...
ErrorCode CPULinearINT8Shadow::execute(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
    ...
    // inputs[1] linear quant output  float 
