...
#include "QNNMatmulNT.hpp"
#include "Types.hpp"
#include "QNNCommonOp.hpp"

namespace mllm {
QNNMatmulNT::QNNMatmulNT(Backend *bn, string opName, bool transpose0, bool transpose1) :
    QNNCommonOp(bn, opName), transpose0_(transpose0), transpose1_(transpose1) {
}

ErrorCode QNNMatmulNT::reshape(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
...
ErrorCode QNNMatmulNT::setUp(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
    ...
    if (name().find("qkv") != string::npos ) {
        ...
        graphAddNode(name(), "MatMul", {inputs[0]->name(), inputs[1]->name()}, outMatmul, paramsMatmul);
        ...
        return graphAddNode(name(), "MatMul", {outQTransposeName, inputs[1]->name()}, out, paramsMatmul);
