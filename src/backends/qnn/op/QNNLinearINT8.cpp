...
ErrorCode QNNLinearINT8::setUp(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
    ...
    case MLLM_TYPE_I8:
        return setUpW8A8(inputs, outputs);
    case MLLM_TYPE_I16:
        return setUpW8A16(inputs, outputs);
    default:
        return NOT_SUPPORT;
    }
}
...
#pragma omp parallel for
    ...
    vector<Qnn_Tensor_t> biasOutput = {{QNN_TENSOR_VERSION_1,
                                        {.v1 = {
                                             ...
                                             .clientBuf = {.data = nullptr,
                                                           .dataSize = 0}}}}};
    return graphAddNode(name() + ".linear_w8a8", "Conv2d", {inputs[0]->name(), weight_.name(), bias_.name()}, biasOutput, params_InceptionV3_InceptionV3_Conv2d_1a_3x3_Conv2D);
}
...
#pragma omp parallel for
    ...
    vector<Qnn_Tensor_t> biasOutput = {{QNN_TENSOR_VERSION_1,
                                        {.v1 = {
                                             ...
                                             .clientBuf = {.data = nullptr,
                                                           .dataSize = 0}}}}};
    return graphAddNode(name() + ".linear_w8a16", "Conv2d", {inputs[0]->name(), weight_.name(), bias_.name()}, biasOutput, params_InceptionV3_InceptionV3_Conv2d_1a_3x3_Conv2D);
}
