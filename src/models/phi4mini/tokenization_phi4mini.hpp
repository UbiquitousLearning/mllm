...
//
#ifndef TOKENIZATION_PHI4_HPP
#define TOKENIZATION_PHI4_HPP
...
class Phi4Tokenizer final : public BPETokenizer {
public:
    explicit Phi4Tokenizer(const std::string &vocab_file, const std::string &merge_file, bool split_special_tokens = false) :
        ...
        chat_template_pre = "<|system|>Your name is Phi, an AI math expert developed by Microsoft.<|end|><|user|>";
        chat_template_end = "<|end|><|assistant|>";
    }
    ...
    Tensor tokenize(const std::string &text, string name = "input", BackendType type = MLLM_CPU) override {
        ...
        static const std::vector<std::string> FIXED_PAT_STRSS = {
            "(?:'[sS]|'[tT]|'[rR][eE]|'[vV][eE]|'[mM]|'[lL][lL]|'[dD])|[^\\r\\n\\p{L}\\p{N}]?\\p{L}+|\\p{N}| ?[^\\s\\p{L}\\p{N}]+[\\r\\n]*|\\s*[\\r\\n]+|\\s+(?!\\S)|\\s+",
        };
        if (split_special_tokens_) {
            const auto word_collection = unicode_regex_split(text, FIXED_PAT_STRSS);
    ...
    std::pair<bool, std::string> postprocess(std::string &text) override {
        if (text == "<|end|>" || text == "<|endoftext|>") return {false, ""};
...
public:
    ...
    token_id_t eos_id_ = 199999, bos_id_ = 199999;
