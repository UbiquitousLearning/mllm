 ...
 *
 */
#ifndef MODELING_QWEN2_5_HPP
#define MODELING_QWEN2_5_HPP

#include "Backend.hpp"
#include "Layer.hpp"
#include "Module.hpp"
#include "Tensor.hpp"
#include "configuration_qwen2_5.hpp"
#include <cmath>
using namespace mllm;

// Copied from GemmaMLP with Gemma->Qwen and using silu
class QWen2_5MLP final : public Module {
public:
    QWen2_5MLP() = default;
    QWen2_5MLP(int hidden_size, int intermediate_size, const QWen2_5NameConfig &names,
...
class QWen2_5Attention final : public Module {
public:
    QWen2_5Attention() = default;
    QWen2_5Attention(const QWen2_5Config &config, const QWen2_5NameConfig &names, const string &base_name) {
...
class QWen2_5Decoder final : public Module {
public:
    QWen2_5Decoder() = default;
    QWen2_5Decoder(const QWen2_5Config &config, const QWen2_5NameConfig &names, const string &base_name) {
        self_atten = QWen2_5Attention(config, names, base_name + names._attn_base_name);
        mlp = QWen2_5MLP(config.hidden_size, config.intermediate_size, names,
    ...
    QWen2_5Attention &get_attention() {
        return self_atten;
    }

private:
    QWen2_5Attention self_atten;
    QWen2_5MLP mlp;
...
class QWen2_5Model final : public Module {
public:
    QWen2_5Model() = default;
    QWen2_5Model(const QWen2_5Config &config, const QWen2_5NameConfig &names, const string &base_name) {
        blocks = List<QWen2_5Decoder>(config.num_hidden_layers, config, names, base_name);
...
private:
    std::vector<QWen2_5Decoder> blocks;
    Layer norm;
};

class QWen2_5ForCausalLM final : public Module {
public:
    QWen2_5ForCausalLM(QWen2_5Config &config) {
        auto names = config.names_config;
        hidden_size = config.hidden_size;
        tie_embedding_words = config.tie_embedding_words;
        embedding = Embedding(config.vocab_size, config.hidden_size, names.token_embd_name);
        model = QWen2_5Model(config, names, names.blk_name);
