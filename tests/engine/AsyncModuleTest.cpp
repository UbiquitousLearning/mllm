...
int main() {
  mllm::initializeContext();
  {
    ...
    // Async run
    auto future = mllm::async::fork(net, Tensor::empty({1, 12, 1024, 1024}, kFloat32).alloc());
    auto outs = mllm::async::wait(future);
    mllm::print(outs[0].shape(), outs[1].shape(), outs[2].shape(), outs[3].shape());
  }
  mllm::memoryReport();
}
