...
void tinyllama(Context* c, int vocab_size= 32000, int hidden_dim= 2048, int ffn_hidden_dim = 5632, int mutil_head_size = 32, int mutil_key_value_head= 4){
    ...
    for(int layer=0; layer<22; ++layer) {
        auto *x = _RMSNorm( {i}, hidden_dim, 1e-5, (string)"model.layers."+std::to_string(layer)+".input_layernorm");
        ...
        x = _RMSNorm( {i}, hidden_dim, 1e-5, (string)"model.layers."+std::to_string(layer)+".post_attention_layernorm");
