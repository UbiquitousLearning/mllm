...
NetTensor *vision_tower(Context* c, NetTensor * i,  int hidden_dim= 1024, int ffn_hidden_dim = 4096, int mutil_head_size = 16, string name = "vision_tower.vision_model"){
    ...
    i = _LayerNorm( {i},  hidden_dim,  true,1e-5, name + ".pre_layrnorm");
    for(int layer=0; layer<23; ++layer) {
        auto *x = _LayerNorm( {i},  hidden_dim,  true,1e-5, name + ".encoder.layers."+std::to_string(layer)+".layer_norm1");
        ...
        x = _LayerNorm( {i}, hidden_dim, true, 1e-5, name + ".encoder.layers."+std::to_string(layer)+".layer_norm2");
    ...
    // i = _LayerNorm( {i}, hidden_dim, true,  1e-5, name + ".post_layernorm");
    return i;
}
...
int main(int argc, char **argv) {
    ...
    cmdParser.add<string>("model", 'm', "specify mllm model path", false, "../models/llava-1.5-7b-fp32.mllm");
