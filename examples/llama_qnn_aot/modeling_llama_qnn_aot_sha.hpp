...
namespace mllm::models::llama3::sha {
...
class LlamaMLP final : public nn::Module {
 ...
 public:
  LlamaMLP() = default;
  LlamaMLP(const std::string& name, const Llama3Config& cfg) : nn::Module(name) {
...
//
// Note: Llama does NOT have RMSNorm after Q/K projection (unlike Qwen3)

class LlamaAttentionSHA final : public nn::Module {
 ...
 public:
  LlamaAttentionSHA() = default;

  LlamaAttentionSHA(const std::string& name, const Llama3Config& cfg) : nn::Module(name) {
    hidden_size_ = cfg.hidden_size;
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    // ========================================================================
    // Llama does NOT have RMSNorm here (unlike Qwen3)
...
class LlamaDecoderSHA final : public nn::Module {
 public:
  int layer_idx_;
  LlamaAttentionSHA self_attn_;
  LlamaMLP mlp_;
  nn::RMSNorm input_layer_norm_;
  nn::RMSNorm post_attention_layer_norm_;

  LlamaDecoderSHA() = default;

  LlamaDecoderSHA(const std::string& name, const Llama3Config& cfg, int layer_idx) : nn::Module(name) {
    layer_idx_ = layer_idx;
    self_attn_ = reg<LlamaAttentionSHA>("self_attn", cfg);
...
class LlamaTextSHA final : public nn::Module {
  nn::ModuleListWithIdx<LlamaDecoderSHA> decode_blocks_;
  nn::RMSNorm norm_;
 ...
 public:
  LlamaTextSHA() = default;

  LlamaTextSHA(const std::string& name, const Llama3Config& cfg) : nn::Module(name) {
    num_hidden_layers_ = cfg.num_hidden_layers;
    hidden_size_ = cfg.hidden_size;
    decode_blocks_ = reg<nn::ModuleListWithIdx<LlamaDecoderSHA>>("layers", cfg.num_hidden_layers, cfg);
...
class LlamaForCausalLM_SHA : public ARGeneration, public nn::Module {
 public:
  explicit LlamaForCausalLM_SHA(const Llama3Config& cfg) : cfg(cfg) {
    eos_token_id_ = cfg.end_of_text_token_id;
    max_length_ = cfg.max_cache_length;
    tie_word_embeddings_ = cfg.tie_word_embeddings;

    llm = reg<LlamaTextSHA>("model", cfg);
...
//
// Note: Llama does NOT have q_norm and k_norm (RMSNorm), so we don't need
 ...
 */
inline void prepareParametersForSHA(const ParameterFile::ptr_t& params, const Llama3Config& cfg) {
