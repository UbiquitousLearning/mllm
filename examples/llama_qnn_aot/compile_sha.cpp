...
#include "modeling_llama_qnn_aot_sha.hpp"
...
MLLM_MAIN({
  ...
  mllm::models::llama3::sha::prepareParametersForSHA(params, model_cfg);
  mllm::print("SHA parameters prepared.");

  // Create SHA model
  auto model = mllm::models::llama3::sha::LlamaForCausalLM_SHA(model_cfg);
...

  {
    ...
    mllm::redirect("llama_qnn_aot_sha_32.mir", [&]() { mllm::print(ir["model"]); });
  }
  ...
  // Model length 1.
  {
    ...
    mllm::redirect("llama_qnn_aot_sha_1.mir", [&]() { mllm::print(ir["model"]); });
  }

  qnn_aot_env.saveContext("context.0", "llama-lpbq-sha.bin");

  mllm::print("SHA compilation completed successfully!");
  mllm::print("Output files:");
  mllm::print("  - llama_qnn_aot_sha_32.mir (IR dump for seq=32)");
  mllm::print("  - llama_qnn_aot_sha_1.mir (IR dump for seq=1)");
  mllm::print("  - llama-lpbq-sha.bin (QNN context)");
});
