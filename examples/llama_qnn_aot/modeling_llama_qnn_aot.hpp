...
class LlamaAttention final : public nn::Module {
  ...
  nn::Conv2D v_proj_;
  nn::Conv2D o_proj_;
  // Llama does NOT have RMSNorm after QK projection (unlike Qwen3)
  nn::CausalMask mask_;
  nn::Softmax softmax_;
 ...
 public:
  ...
  LlamaAttention(const std::string& name, const Llama3Config& cfg) : nn::Module(name) {
    hidden_size_ = cfg.hidden_size;
...
class LlamaDecoder final : public nn::Module {
 public:
  ...
  LlamaDecoder(const std::string& name, const Llama3Config& cfg, int layer_idx) : nn::Module(name) {
...
class LlamaText final : public nn::Module {
  nn::ModuleListWithIdx<LlamaDecoder> decode_blocks_;
  nn::RMSNorm norm_;
 ...
 public:
  ...
  LlamaText(const std::string& name, const Llama3Config& cfg) : nn::Module(name) {
    num_hidden_layers_ = cfg.num_hidden_layers;
    hidden_size_ = cfg.hidden_size;
    decode_blocks_ = reg<nn::ModuleListWithIdx<LlamaDecoder>>("layers", cfg.num_hidden_layers, cfg);
...
class LlamaForCausalLM : public ARGeneration, public nn::Module {
 public:
  explicit LlamaForCausalLM(const Llama3Config& cfg) : cfg(cfg) {
    eos_token_id_ = cfg.end_of_text_token_id;
    max_length_ = cfg.max_cache_length;
    tie_word_embeddings_ = cfg.tie_word_embeddings;

    llm = reg<LlamaText>("model", cfg);
