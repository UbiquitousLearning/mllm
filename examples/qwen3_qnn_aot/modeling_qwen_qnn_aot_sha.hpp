...
//
// This file implements the MHA -> SHA (Multi-Head Attention to Single-Head Attention)
// transformation for QNN AOT compilation, similar to ExecuTorch's ConvertMhaToSha pass.
//
 ...
 public:
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    for (int h = 0; h < num_attention_heads_; ++h) {
      ...
      q_h = q_h.view({1, -1, 1, head_dim_}, /*ssa=*/true).transpose(1, 2);
    ...
    for (int h = 0; h < num_key_value_heads_; ++h) {
      ...
      k_h = k_h.view({1, -1, 1, head_dim_}, /*ssa=*/true).transpose(1, 2);
    ...
    for (int h = 0; h < num_key_value_heads_; ++h) {
      ...
      v_h = v_h.view({1, -1, 1, head_dim_}, /*ssa=*/true).transpose(1, 2);
      ...
      if (params->has(orig_weight_name)) {
        ...
        for (int h = 0; h < num_heads; ++h) {
          ...
          if (has_scale) {
            ...
            params->push(new_scale_name, orig_scale.contiguous());
          }

          // Zero point: copy (scalar)
          if (has_zp) {
            std::string new_zp_name = layer_prefix + "q_norm." + h_str + ".zero_point";  // NOLINT
            params->push(new_zp_name, orig_zp.contiguous());
          }
        }
      }
    }
    ...
    auto copyQDQParams = [&](const std::string& base_name, const std::string& new_base_name, int count) {
      ...
      if (params->has(scale_name)) {
        ...
        for (int h = 0; h < count; ++h) {
          ...
          params->push(new_scale_name, scale.contiguous());
          params->push(new_zp_name, zp.contiguous());
        }
      }
    };
