...
//
// This file demonstrates the MHA -> SHA (Multi-Head Attention to Single-Head Attention)
// optimization for QNN AOT compilation. Similar to ExecuTorch's ConvertMhaToSha pass,
// this approach splits large Q/K/V projections into per-head projections.
//
// Benefits:
...
MLLM_MAIN({
  ...
  mllm::redirect("qwen3_qnn_aot_sha.mir", [&]() { mllm::print(ir["model"]); });
  ...
  mllm::print("  - qwen3_qnn_aot_sha.mir (IR dump)");
