...
class Qwen2Attention final : public nn::Module {
  nn::Conv2D q_proj_;
  nn::Conv2D k_proj_;
  nn::Conv2D v_proj_;
  nn::Conv2D o_proj_;
  // Qwen2 does NOT have RMSNorm after QK projection (unlike Qwen3)
  nn::CausalMask mask_;
  nn::Softmax softmax_;
 ...
 public:
  ...
  Qwen2Attention(const std::string& name, const qwen3::Qwen3Config& cfg) : nn::Module(name) {
    hidden_size_ = cfg.hidden_size;
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    query_states = ptq::QDQ(this, query_states, "q_proj_output_qdq");
    key_states = ptq::QDQ(this, key_states, "k_proj_output_qdq");
...
class Qwen2Decoder final : public nn::Module {
 public:
  int layer_idx_;
  Qwen2Attention self_attn_;
  Qwen2MLP mlp_;
  ...
  Qwen2Decoder(const std::string& name, const qwen3::Qwen3Config& cfg, int layer_idx) : nn::Module(name) {
    layer_idx_ = layer_idx;
    self_attn_ = reg<Qwen2Attention>("self_attn", cfg);
...
class Qwen2Text final : public nn::Module {
  nn::ModuleListWithIdx<Qwen2Decoder> decode_blocks_;
  nn::RMSNorm norm_;
 ...
 public:
  ...
  Qwen2Text(const std::string& name, const qwen3::Qwen3Config& cfg) : nn::Module(name) {
    num_hidden_layers_ = cfg.num_hidden_layers;
    hidden_size_ = cfg.hidden_size;
    decode_blocks_ = reg<nn::ModuleListWithIdx<Qwen2Decoder>>("layers", cfg.num_hidden_layers, cfg);
...
class Qwen2ForCausalLM : public ARGeneration, public nn::Module {
 public:
