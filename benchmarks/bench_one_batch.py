"""SGLang-style one-batch benchmark for pymllm (delivery copy).
...
Delivery hardening vs ``pymllm/bench_one_batch.py``: the original warms up only
the FIRST setting and measures every (batch, in, out) point exactly ONCE, so the
single-shot prefill latency at each new input length includes cold-start /
autotune cost and swings 20-30% run-to-run.  This copy adds, per setting,
``--bench-repeat`` measured runs PLUS one dedicated warmup at that exact setting,
and reports the MEDIAN prefill/decode throughput.  Decode was already a median
over output steps; this makes prefill equally robust and deliverable.
"""
...
def add_bench_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    group.add_argument(
        "--bench-repeat",
        type=_positive_int,
        default=BenchArgs.bench_repeat,
        help="Measured repeats per setting; the median is reported (default: 3). "
             "Each setting also gets one dedicated warmup before timing.",
    )
    return parser
...
def _bench_args_from_namespace(namespace: argparse.Namespace) -> BenchArgs:
    return BenchArgs(
        ...
        bench_repeat=namespace.bench_repeat,
    )
...
def _aggregate_repeats(
    ...
    """Median-aggregate ``bench_repeat`` measured runs of one setting.

    Median (not mean) is used so a single cold/jittery run cannot skew the
    reported throughput -- the same robustness the per-step decode median
    already provides, now extended to the per-setting prefill.
    """
    base = dict(reps[-1])
    pref = [r["prefill_throughput"] for r in reps]
    dec = [r["median_decode_throughput"] for r in reps]
    pref_ms = [r["prefill_latency"] for r in reps]
    base["prefill_throughput"] = float(statistics.median(pref))
    base["median_decode_throughput"] = float(statistics.median(dec))
    base["prefill_latency"] = float(statistics.median(pref_ms))
    base["bench_repeat"] = bench_repeat
    base["prefill_throughput_samples"] = [round(x, 2) for x in pref]
    base["decode_throughput_samples"] = [round(x, 2) for x in dec]
    return base
...
def run_benchmark(cfg: GlobalConfig, args: BenchArgs) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for index, setting in enumerate(settings):
            logger.info(
                "Benchmark: batch_size=%d input_len=%d output_len=%d (repeat=%d)",
                setting.batch_size,
                setting.input_len,
                setting.output_len,
                args.bench_repeat,
            )
            # Dedicated warmup at THIS exact setting so the first measured
            # prefill is not polluted by cold-start / autotune at a new length.
            run_single_setting(
                bench_runner=bench_runner,
                args=args,
                setting=setting,
            ...
            result = _aggregate_repeats(reps, setting, args.bench_repeat)
