...
def get_clip_and_scale(act_dict: dict, t01m_thre=5, clip_all=False, 
                       no_clip_input=None, no_clip_output=None) -> tuple:
    """
    ...
    no_clip_input = no_clip_input if no_clip_input is not None else set()
    no_clip_output = no_clip_output if no_clip_output is not None else set()
    ...
    for i in stat:
        ...
        clip_top[i] = {}
        # layer input
        if (not (i in no_clip_input)) and (clip_all or top_0_1_input * t01m_thre > ori_scale[i]["input"]):
            clip_input_num += 1
        ...
        # layer output
        if (not (i in no_clip_output)) and (clip_all or top_0_1_output * t01m_thre > ori_scale[i]["output"]):
            clip_output_num += 1
