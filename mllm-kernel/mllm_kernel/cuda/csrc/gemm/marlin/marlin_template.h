...
__device__ inline void mma(const typename ScalarType<scalar_t>::FragA& a_frag,
                           const typename ScalarType<scalar_t>::FragB& frag_b, typename ScalarType<scalar_t>::FragC& frag_c) {
...
template<typename scalar_t>
__device__ inline void sub_zp(typename ScalarType<scalar_t>::FragB& frag_b, typename ScalarType<scalar_t>::scalar_t2& frag_zp,
                              int i) {
  using scalar_t2 = typename ScalarType<scalar_t>::scalar_t2;
...
template<typename scalar_t>
__device__ inline void scale4(typename ScalarType<scalar_t>::FragB& frag_b, typename ScalarType<scalar_t>::FragS& frag_s_1,
                              typename ScalarType<scalar_t>::FragS& frag_s_2, typename ScalarType<scalar_t>::FragS& frag_s_3,
...
__global__ void Marlin(const int4* __restrict__ A,               // fp16 input matrix of shape mxk
  ...
  constexpr bool dequant_skip_flop = !is_int_type || has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value
  ...
  auto init_slice = [&](bool first_init = false) {
    ...
    if (parallel * n_tiles >= gridDim.x) {
      if (slice_count > 1 && slice_idx == slice_count - 1) { locks_off++; }
    } else {
      locks_off++;
    }
...
#pragma unroll
  ...
  auto fetch_act_order_scales_to_shared = [&](bool is_async, int first_group_id, int last_group_id) {
    ...
    if (sh_num_groups > act_s_max_num_groups) { sh_num_groups = act_s_max_num_groups; }

    if (sh_first_group_id + sh_num_groups > num_groups) { sh_num_groups = num_groups - sh_first_group_id; }
...
#pragma unroll
      for (int i = 0; i < b_sh_wr_iters; i++) {
#pragma unroll
        for (int j = 0; j < b_thread_vecs; j++) { cp_async4(&sh_b_stage[b_sh_wr_delta * i + b_sh_wr + j], B_ptr[i] + j); }
      ...
      if constexpr (has_act_order) {
        ...
        if (cur_k < prob_k && cur_k < slice_k_finish) {
          ...
          if (threadIdx.x < g_idx_stage) { cp_async4_pred(&sh_g_idx_stage[threadIdx.x], &cur_g_idx_stage_ptr[threadIdx.x]); }
        }
      } else {
        if constexpr (group_blocks != -1) {
          ...
          if constexpr (group_blocks >= thread_k_blocks) {
            ...
            for (int i = 0; i < s_tb_groups; i++) {
              if (s_sh_wr_pred) { cp_async4(&sh_s_stage[i * s_sh_stride + s_sh_wr], &scales_ptr[s_gl_rd]); }
              s_gl_rd += s_gl_rd_delta;
            }
          }
        }
        ...
        if constexpr (has_zp && group_blocks != -1) {
          ...
          if constexpr (group_blocks >= thread_k_blocks) {
            ...
            for (int i = 0; i < zp_tb_groups; i++) {
              if (zp_sh_wr_pred) { cp_async4(&sh_zp_stage[i * zp_sh_stride + zp_sh_wr], &zp_ptr[zp_gl_rd]); }
              zp_gl_rd += zp_gl_rd_delta;
            }
          }
        }
  ...
  auto fetch_col_zp_to_shared = [&]() {
    if (zp_sh_wr_pred) { cp_async4(&sh_zp[zp_sh_wr], &zp_ptr[zp_gl_rd]); }
  };
...
#pragma unroll
  ...
  auto init_same_group = [&](int pipe) {
    if constexpr (!has_act_order) { return; }
  ...
  auto fetch_scales_to_registers = [&](int k, int full_pipe) {
    ...
    if (cur_k >= prob_k || cur_k >= slice_k_finish) { return; }
  ...
  auto fetch_zp_to_registers = [&](int k, int full_pipe) {
    ...
    if constexpr (has_zp && !is_zp_float) {
      ...
      if constexpr (group_blocks == -1) {
        ...
        if (k == 0 && full_pipe == 0) {
#pragma unroll
          for (int i = 0; i < num_ints_per_thread; i++) { frag_qzp[k % 2][i] = (reinterpret_cast<int*>(sh_zp))[zp_sh_rd + i]; }
        }
...
#pragma nv_diagnostic pop
  ...
  auto dequant_data = [&](int q, scalar_t2* frag_b_ptr) { dequant<scalar_t2, w_type_id, dequant_skip_flop>(q, frag_b_ptr); };
  ...
  auto matmul = [&](int k) {
    ...
    if constexpr (!dequant_skip_flop && has_zp && is_zp_float) {
      if (is_new_zp) { reinterpret_cast<int4*>(&frag_zp)[0] = reinterpret_cast<int4*>(&frag_zpf[k2])[0]; }
    }
...
#pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      if constexpr (has_act_order) {
        ...
        scale4<scalar_t>(frag_b0, act_frag_s[k2][0][j], act_frag_s[k2][1][j], act_frag_s[k2][2][j], act_frag_s[k2][3][j], 0);
        scale4<scalar_t>(frag_b1, act_frag_s[k2][0][j], act_frag_s[k2][1][j], act_frag_s[k2][2][j], act_frag_s[k2][3][j], 1);
...
#pragma unroll
                for (int k = 0; k < 4; k++) reinterpret_cast<FragC*>(frag_c)[4 * 2 * m_block + j][k] += c_rd[k] + c_wr[k];
              }
...
#pragma unroll
            for (int j = 0; j < 2 * 4; j++) {
              int delta = 0;
              if constexpr (m_block_size_8) { delta = j % 2 == 1 ? -2 : 0; }
...
#pragma unroll
        for (int f = 0; f < 4; f++) { frag_c_ptr[k * 4 + f] += sh_c_ptr[f]; }
      }
    }
...
#pragma unroll
  ...
  auto write_result = [&]() {
    ...
    auto write = [&](int idx, float c0, float c1, FragS& s) {
      ...
      if constexpr (w_type == host::kFE2M1f) { res = __hmul2(res, global_scale); }
...
#pragma unroll
          for (int a = 0; a < 4; a++) { atomicAdd(&C_half2[a], sh_red_half2[a]); }
        } else {
...
#pragma unroll
    for (int i = 0; i < stages - 1; i++) {
      if (has_act_order && i == 0) {
        ...
        if (last_g_idx >= prob_k) { last_g_idx = prob_k - 1; }
      ...
      if constexpr (has_zp && !is_zp_float && group_blocks == -1) {
        if (i == 0) {
          ...
          if constexpr (!dequant_skip_flop) { fetch_col_scale_to_shared(); }
        }
      }
    ...
    if constexpr (has_act_order) { slice_k_start_shared_fetch += tb_k * (stages - 1); }
  };
  if (slice_iters) { start_pipes(); }
