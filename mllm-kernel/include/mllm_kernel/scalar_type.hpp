...
class ScalarType {
 public:
  ...
  static constexpr ScalarType int_(uint8_t size_bits, int32_t bias = 0) { return ScalarType(0, size_bits - 1, true, bias); }

  static constexpr ScalarType uint(uint8_t size_bits, int32_t bias = 0) { return ScalarType(0, size_bits, false, bias); }
 ...
 private:
  ...
  static constexpr auto id_size_bits() {
    return reduce_member_types([](int acc, auto member) -> int { return acc + member_id_field_width<decltype(member)>(); }, 0);
  }
 ...
 public:
  ...
  constexpr bool is_ieee_754() const { return is_floating_point() && finite_values_only == false && nan_repr == NAN_IEEE_754; }
 ...
 private:
  double _floating_point_max() const {
    ...
    if (nan_repr == NAN_EXTD_RANGE_MAX_MIN) { max_mantissa -= 1; }
 ...
 public:
  std::string str() const {
    if (is_floating_point()) {
      ...
      if (has_bias()) { ret += "b" + std::to_string(bias); }
      return ret;
    }
  }
