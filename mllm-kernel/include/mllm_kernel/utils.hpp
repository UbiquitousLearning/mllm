...
struct DebugInfo : public source_location_t {
  DebugInfo(source_location_t loc = source_location_t::current()) : source_location_t(loc) {}  // NOLINT
};

struct PanicError : public std::runtime_error {
 public:
  explicit PanicError(const std::string& msg) : runtime_error(msg), message_(msg) {}
  [[nodiscard]] auto root_cause() const -> std::string_view {
    const auto str = std::string_view{message_};
...
struct RuntimeCheck {
  ...
  explicit RuntimeCheck(Cond&& condition, Args&&... args, DebugInfo location = {}) {
    if (condition) return;
    [[unlikely]] ::mllm_kernel::host::panic(location, std::forward<Args>(args)...);
  }
  template<typename Cond>
...
template<typename... Args>
struct Panic {
  explicit Panic(Args&&... args, DebugInfo location = {}) { ::mllm_kernel::host::panic(location, std::forward<Args>(args)...); }
  explicit Panic(DebugInfo location, Args&&... args) { ::mllm_kernel::host::panic(location, std::forward<Args>(args)...); }
