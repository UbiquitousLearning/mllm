...
int32_t isOk(MllmCAny ret) {
  ...
  if (ret.type_id == kCustomObject && ret.v_custom_ptr != nullptr) return true;
  return false;
}
...
void setLogLevel(int level) { mllm::setLogLevel(static_cast<mllm::LogLevel>(level)); }
...
MllmCAny createQwen3ProbingSession(const char* model_path, const char* probe_path) {
  if (model_path == nullptr || probe_path == nullptr) {
    printf("[C++ Service] createQwen3ProbingSession error: invalid arguments.\n");
    return MllmCAny{.type_id = kRetCode, .v_return_code = -1};
  }
  try {
    auto qwen3_session = std::make_shared<mllm::models::qwen3_probing::Qwen3ProbingSession>();
    qwen3_session->fromPreTrain(model_path);

    mllm::models::qwen3_probing::ProbingArgs p_args;
    p_args.enable_prefill_check = true;
    p_args.prefill_stop_threshold = 0.7f;
    p_args.default_prefill_layers = {27, 30};
    ...
    qwen3_session->loadProbes(probe_path, p_args);
    ...
    printf("[C++ Service] createQwen3ProbingSession exception: %s\n", e.what());
    return MllmCAny{.type_id = kRetCode, .v_return_code = -1};
  }
}
...
const char* pollResponse(const char* session_id) {
  if (session_id == nullptr) { return nullptr; }
  ...
  // Always return the chunk to upper layers, including the final chunk with
  // finish_reason="stop". The HTTP layer already handles stop detection and
  // stream termination; filtering it here can drop valid assistant output.
...
void freeResponseString(const char* response_str) {
  if (response_str != nullptr) { delete[] response_str; }
}