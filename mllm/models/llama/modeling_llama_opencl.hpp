...
class LlamaForCausalLM : public nn::Module, public ARGeneration {
 public:
  ...
  explicit LlamaForCausalLM(const std::string& name, const LLaMAConfig& cfg, bool mask_by_tensor = false)
    ...
    inv_freq_ = makeRoPEInvFreq(cfg.hidden_size / cfg.num_attention_heads, cfg.rope_theta);
  ...
  ARGenerationOutputPast forward(const ARGenerationOutputPast& input, const ARGenerationArgs& args) override {
    ...
    llm_embedding_sin = llm_embedding_sin.to(device());
    llm_embedding_cos = llm_embedding_cos.to(device());
  ...
  LlamaText model;
  nn::Linear lm_head_;
  Tensor inv_freq_;  // directly hold inv_freq buffer, avoid using registerBuffer
