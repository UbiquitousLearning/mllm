...
struct Qwen2_5VLConfig : protected ConfigFile {
  Qwen2_5VLConfig() = default;

  explicit Qwen2_5VLConfig(const std::string& file_path) : ConfigFile(file_path) {
    // Init all
    visual_in_chans = data()["visual_in_chans"];
    visual_hidden_size = data()["visual_hidden_size"];
    ...
    visual_out_hidden_size = data()["visual_out_hidden_size"];
    visual_mlp_ratio = data()["visual_mlp_ratio"];
    visual_num_heads = data()["visual_num_heads"];
    visual_depth = data()["visual_depth"];
    visual_intermediate_size = data()["visual_intermediate_size"];
    visual_fullatt_block_indexes = data()["visual_fullatt_block_indexes"].get<std::vector<int32_t>>();
    visual_window_size = data()["visual_window_size"];
  ...
  int32_t visual_out_hidden_size = 2048;
