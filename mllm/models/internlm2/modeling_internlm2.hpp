...
class InternLM2Attention final : public nn::Module {
 ...
 public:
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    auto qkv_parts = nn::functional::split(qkv, {num_key_value_groups_, 1, 1}, 3);
    auto query_states = qkv_parts[0].contiguous().view({B, S, num_attention_heads_, head_dim_}).permute({0, 2, 1, 3});  // [B, num_heads, S, D]
    auto key_states = qkv_parts[1].contiguous().squeeze(3).permute({0, 2, 1, 3});  // [B, num_kv_heads, S, D]
    auto value_states = qkv_parts[2].contiguous().squeeze(3).permute({0, 2, 1, 3});
...
class InternLM2ForCausalLM : public ARGeneration, public nn::Module {
 public:
  explicit InternLM2ForCausalLM(const InternLM2Config& cfg)
    ...
    lm_head_ = reg<nn::Linear>("output", cfg.hidden_size, cfg.vocab_size, cfg.bias, aops::LinearImplTypes::kDefault);
