  ...
  for (int b = 0; b < batch_size; ++b) {
    for (int s = 0; s < seq_len; ++s) {
      ...
      for (int d = 0; d < inv_freq_len; ++d) {
        freqs_ptr[b * seq_len * inv_freq_len + s * inv_freq_len + d] =
            static_cast<float>(pos) * inv_freq_ptr[d];
      }
    }
  }
...
class Smollm3MLP final : public nn::Module {
 ...
 public:
  Smollm3MLP() = default;
  Smollm3MLP(const std::string& name, const Smollm3Config& cfg) : nn::Module(name) {
...
class Smollm3Attention final : public nn::Module {
  nn::Linear q_proj_;
  nn::Linear k_proj_;
  nn::Linear v_proj_;
  nn::Linear o_proj_;
 ...
 public:
  Smollm3Attention() = default;

  Smollm3Attention(const std::string& name, const Smollm3Config& cfg) : nn::Module(name) {
...
class Smollm3Decoder final : public nn::Module {
 public:
  ...
  Smollm3Decoder() = default;
  Smollm3Decoder(const std::string& name, const Smollm3Config& cfg) : nn::Module(name) {
    self_attn_ = reg<Smollm3Attention>("self_attn", cfg);
    mlp_ = reg<Smollm3MLP>("mlp", cfg);
...
class Smollm3Text final : public nn::Module {
  nn::ModuleList<Smollm3Decoder> decode_blocks_;
  nn::RMSNorm norm_;
  nn::Embedding embedding_;

 public:
  Smollm3Text() = default;
  Smollm3Text(const std::string& name, const Smollm3Config& cfg) : nn::Module(name) {
    decode_blocks_ = reg<nn::ModuleList<Smollm3Decoder>>("layers", cfg.num_hidden_layers, cfg);
