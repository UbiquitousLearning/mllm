...
public:
    ...
    explicit ImageSliceProcessor(int max_slice_nums = 9, int scale_resolution = 448, int patch_size = 14) :
        max_slice_nums_(max_slice_nums),
        scale_resolution_(scale_resolution),
    ...
    std::vector<int> get_sliced_grid(int width, int height, bool never_split = false){
      ...
      for (int split_num : candidate_nums){
        for(int m = 1; m <= split_num; ++m){
          if(split_num % m == 0){
            candidate_grids.push_back({m, split_num/m});
          }
        }
      }
    ...
    Image crop_image(Image& image, int x, int y, int crop_width, int crop_height) {
        ...
        for (int row = 0; row < crop_height; ++row) {
            ...
            std::memcpy(crop_data + crop_row_offset,
                       src_data + src_row_offset,
                       crop_width * src_channels);
        }
...
public:
    explicit MiniCPMOImageProcessor(int patch_size = 14, int image_size = 980,
                                   float mean_0 = 0.5, float mean_1 = 0.5, float mean_2 = 0.5,
                                   float std_0 = 0.5, float std_1 = 0.5, float std_2 = 0.5) :
      patch_size_(patch_size),
    ...
    std::string get_slice_image_placeholder(std::pair<int, int> image_size, const std::vector<int> &grid, int image_idx = 0, bool use_image_id = true){
      std::string image_placeholder = "<image>";
      for(int i=0; i < image_feature_size; i++){
        image_placeholder += "<unk>";
      }
    ...
    std::pair<std::vector<int64_t>, std::vector<std::pair<int,int>>> calc_bounds(const std::vector<int64_t> &input_ids,
       ...
       for(int i = 0; i < seq_len; ++i){
        ...
        if(token_id ==im_start_id || token_id == slice_start_id){
          image_start_positions.push_back(i);
        }
        if(token_id ==im_end_id || token_id == slice_end_id){
          image_end_positions.push_back(i);
        }
      }
        ...
        for(int i = 0; i < valid_image_nums && i < image_start_positions.size() && i < image_end_positions.size(); ++i){
          image_bounds.push_back({image_start_positions[i], image_end_positions[i]});
        }
    ...
    std::tuple<std::vector<Tensor>, std::vector<std::pair<int, int>>, std::vector<std::pair<int, int>>, std::vector<int>> process(const std::string& image_path, int max_slice_nums = 9) {
        ...
        if (!patches.empty()) {
            for (const auto& patch_row : patches) {
                for (const auto& patch : patch_row) {
                    slice_images.push_back(patch);
                }
            }
        }
        ...
        for (auto& slice_img : slice_images) {
            ...
            tgt_sizes.push_back({patch_h, patch_w});
        }
                ...
                for (int pw = 0; pw < num_patches_w; ++pw) {
                    ...
                    for (int kh = 0; kh < patch_size_; ++kh) {
                        for (int kw = 0; kw < patch_size_; ++kw) {
                            ...
                            *output.offsettedPtr<float>({c, kh, output_col}) =
                                input_tensor.at<float>({c, img_h, img_w});
                        }
                    }
                }
            }
