...
inline auto makeMultimodalPositionEmbedding(Tensor& position_ids, const Tensor& inv_freq, int seq_len, int output_dim,
  ...
  if (!mrope_section.empty()) {
    ...
    // When mrope_section is empty, we still need to reshape from 3D to 2D for MultimodalRoPE
    // The MultimodalRoPE operation expects [seq_len, dim] format
    int num_rows = tmp_sin.shape()[1];  // seq_len
    int num_cols = tmp_sin.shape()[2];  // dim

    sin = Tensor::empty({num_rows, num_cols}, kFloat32, kCPU).alloc();
    cos = Tensor::empty({num_rows, num_cols}, kFloat32, kCPU).alloc();

    // Copy data from the first dimension (dimension 0) of tmp_sin/cos to the output
    // This is a simplified approach - we take the first "layer" of the 3D tensor
    for (int row = 0; row < num_rows; ++row) {
      auto src_sin_ptr = tmp_sin.offsettedPtr<float>({0, row, 0});
      auto src_cos_ptr = tmp_cos.offsettedPtr<float>({0, row, 0});
      auto dst_sin_ptr = sin.offsettedPtr<float>({row, 0});
      auto dst_cos_ptr = cos.offsettedPtr<float>({row, 0});

      for (int col = 0; col < num_cols; ++col) {
        dst_sin_ptr[col] = src_sin_ptr[col];
        dst_cos_ptr[col] = src_cos_ptr[col];
      }
    }
  }
...
class Qwen2VLText final : public nn::Module {
 ...
 public:
  ...
  Qwen2VLText(const std::string& name, const Qwen2VLConfig& cfg) : nn::Module(name) {
    ...
    decode_blocks_ = reg<nn::ModuleList<Qwen2VLDecoder>>("model.layers", cfg.num_hidden_layers, cfg);
    for (auto [idx, b] : enumerate(decode_blocks_.list())) { b.self_attn_.layer_idx_ = idx; }

    norm_ = reg<nn::RMSNorm>("model.norm", cfg.rms_norm_eps);
...
class Qwen2VLForCausalLM : public ARGeneration {
 public:
  explicit Qwen2VLForCausalLM(const Qwen2VLConfig& cfg) : cfg(cfg), llm("llm", cfg), visual("visual", cfg) {
