...
public:
    ...
    inline SiglipVisionEmbeddings(const std::string& name, const MiniCPMOConfig& config) : nn::Module(name) {
        ...
        patch_embedding_ = reg<nn::Conv2D>("patch_embedding", 3, embed_dim_,
                                          std::vector<int32_t>{patch_size_, patch_size_},
                                          std::vector<int32_t>{patch_size_, patch_size_},
                                          std::vector<int32_t>{0, 0}, std::vector<int32_t>{1, 1}, 1, true);
    ...
    std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
        ...
        if (!tgt_sizes.isNil() && !patch_attention_mask.isNil()) {
             ...
             for (int i = 1; i < num_patches_per_side_; ++i) {
             ...
             for (int batch_idx = 0; batch_idx < batch_size; batch_idx++) {
                ...
                for (int i = 0; i < nb_patches_h; ++i) {
                    fractional_coords_h.push_back(i * step_h);
                }
                for (int i = 0; i < nb_patches_w; ++i) {
                    fractional_coords_w.push_back(i * step_w);
                }
...
public:
    ...
    std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
        ...
        // Reshape for multi-head attention: [B, seq_len, embed_dim] -> [B, seq_len, num_heads, head_dim] -> [B, num_heads, seq_len, head_dim]
...
public:
    ...
    inline SiglipVisionEncoder(const std::string& name, const MiniCPMOConfig& config) : nn::Module(name) {
        ...
        for (int i = 0; i < num_layers_; ++i) {
            layers_.push_back(reg<SiglipEncoderLayer>("layers." + std::to_string(i), config));
        }
    }

    std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
        auto inputs_embeds = inputs[0];
        auto attention_mask = inputs.size() > 1 ? inputs[1] : Tensor::nil();
    ...
    std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
        ...
        for(int i=0;i<batch_size;i++){
          for(int j=0;j<max_patches;j++){
            patch_attention_mask.at<uint8_t>({i,0,j}) = 0;
          }
          if(!tgt_sizes.isNil() && i<tgt_sizes.shape()[0]){
            ...
            for(int j=0;j<valid_patches && j<max_patches;j++){
              patch_attention_mask.at<uint8_t>({i,0,j}) = 1;
            }
          }
        }
