...
public:
    ...
    ResamplerAttention(const std::string& name, int32_t embed_dim, int32_t num_heads)
        ...
        in_proj_weight_ = reg<nn::Param>("in_proj_weight", getModuleName() + ".in_proj.weight",
                                        Tensor::shape_t{3 * embed_dim_, embed_dim_});
        in_proj_bias_ = reg<nn::Param>("in_proj_bias", getModuleName() + ".in_proj.bias",
                                      Tensor::shape_t{3 * embed_dim_});
        out_proj_ = reg<nn::Linear>("out_proj", embed_dim_, embed_dim_, true);
    }

    std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
        auto query = inputs[0];  // [num_queries, embed_dim]
        auto key = inputs[1];    // [seq_len, embed_dim]
        ...
        for(int i=0; i<num_queries; i++){
            for(int j=0; j<embed_dim_; j++){
                *q.offsettedPtr<float>({i, j}) += q_bias.at<float>({j});
            }
        }
            ...
            for (int32_t h = 0; h < num_heads_; ++h) {
                for (int32_t q_idx = 0; q_idx < num_queries; ++q_idx) {
                    for (int32_t s = 0; s < seq_len; ++s) {
                        if (key_padding_mask.at<uint8_t>({s}) == 1) {
                            *attn_weights.offsettedPtr<float>({h, q_idx, s}) = mask_value;
                        }
                    }
                }
            }
...
public:
    ...
    Resampler(const std::string& name, int32_t num_queries, int32_t embed_dim, int32_t num_heads,
              int32_t kv_dim = -1, const std::vector<int32_t>& max_size = {70, 70})
        : nn::Module(name), num_queries_(num_queries), embed_dim_(embed_dim),
          num_heads_(num_heads), kv_dim_(kv_dim == -1 ? embed_dim : kv_dim), max_size_(max_size) {
    ...
    std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
        ...
        for(int i = 0; i < batch_size; i++){
            for(int j = 0; j < max_patch_len; j++){
                key_padding_mask.at<uint8_t>({i,j}) = 1;  
            }
            for(int j = 0; j < patch_len[i] && j < max_patch_len; j++){
                key_padding_mask.at<uint8_t>({i,j}) = 0;  
            }
        }
        ...
        for(int i = 0; i < batch_size; i++){
            ...
            for(int j = actual_len; j < max_patch_len; j++){
                for(int k = 0; k < embed_dim_; k++){
                    *pos_embed_padded.offsettedPtr<float>({i, j, k}) = 0.0f;
                }
            }
        }
        ...
        for (int32_t b = 0; b < batch_size; ++b) {
            ...
            for(int i = 0; i < seq_len; i++){
                for(int j = 0; j < embed_dim_; j++){
                    x_b.at<float>({i, j}) = x.at<float>({b, i, j});
                }
            }
            ...
            Tensor key_padding_mask_b = Tensor::empty({max_patch_len}, kUInt8).alloc();
              for(int i = 0; i < max_patch_len; i++){
                  key_padding_mask_b.at<uint8_t>({i}) = key_padding_mask.at<uint8_t>({b, i});
              }
            ...
            auto attn_output = has_padding
                ? attn_(q, kv_input, x_b, key_padding_mask_b)[0]
                : attn_(q, kv_input, x_b)[0];
        ...
        for(int i = 0; i < batch_size; i++){
            ...
            for(int j = 0; j < num_queries_; j++){
                for(int k = 0; k < embed_dim_; k++){
                    *out_tensor.offsettedPtr<float>({i, j, k}) = out_i.at<float>({j, k});
                }
            }
        }
