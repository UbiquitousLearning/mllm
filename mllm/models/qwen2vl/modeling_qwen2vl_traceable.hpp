...
class PatchEmbed final : public nn::Module {
 ...
 public:
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    hidden_states = hidden_states.view({-1, in_chans_, temporal_patch_size_, patch_size_, patch_size_}, true);
...
class VisionAttention final : public nn::Module {
 ...
 public:
  ...
  Tensor applyVisionRoPEPrimitive(Tensor x, Tensor visual_embedding_sin, Tensor visual_embedding_cos) {
    const int32_t half_dim = head_dim_ / 2;
    auto x1 = x.slice({kAll, kAll, kAll, {kAll, half_dim}}, true);
    auto x2 = x.slice({kAll, kAll, kAll, {half_dim, kAll}}, true);
    ...
    return nn::functional::concat({y1, y2}, -1);
  }
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    auto query_states = qkv_states.slice({kAll, {0, 1}, kAll, kAll}, true).transpose(0, 1);
    auto key_states = qkv_states.slice({kAll, {1, 2}, kAll, kAll}, true).transpose(0, 1);
    auto value_states = qkv_states.slice({kAll, {2, 3}, kAll, kAll}, true).transpose(0, 1);
...
class Qwen2VLAttention final : public nn::Module {
 ...
 public:
  ...
  Qwen2VLAttention(const std::string& name, const Qwen2VLConfig& cfg) : nn::Module(name) {
    ...
    // Register KVCache layer
    kv_cache_ = reg<nn::KVCache>("kv_cache", layer_idx_, num_attention_heads_, num_key_value_heads_, head_dim_, false);
  }
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    const auto& k = kv_outputs[0];
    const auto& v = kv_outputs[1];
    key_states = k;
    value_states = v;
...
class Qwen2VLDecoder final : public nn::Module {
 public:
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    x = self_attn_(x, llm_embedding_sin, llm_embedding_cos)[0];
    auto tmp = x + inputs[0];
...
class Qwen2VLForCausalLM : public ARGeneration {
 public:
  ...
  ARGenerationOutputPast forward(const ARGenerationOutputPast& input, const ARGenerationArgs& args) override {
    ...
    sequence = llm(input_embeddings, llm_embedding_sin, llm_embedding_cos)[0];
  ...
  IROutput trace(const ARGenerationOutputPast& input, const ARGenerationArgs& args) override {
    ...
    if (input.count("img")) {
      ...
      // NOTE:
      // Trace. Continue to trace SigLIP.
    ...
    sequence = ir::lowlevel::traceModule(llm, input_embeddings, llm_embedding_sin, llm_embedding_cos)[0];
