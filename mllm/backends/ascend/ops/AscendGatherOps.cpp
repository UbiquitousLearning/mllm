...
AscendGatherOp::AscendGatherOp(const aops::GatherOpOptions& options) : aops::GatherOp(options) {}

void AscendGatherOp::reshape(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs) {
  aops::GatherOp::reshape(inputs, outputs);
}

void AscendGatherOp::setup(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs) {
  BaseOp::setup(inputs, outputs);
}

void AscendGatherOp::forward(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs) {
  MLLM_RT_ASSERT_EQ(inputs.size(), 2);
  MLLM_RT_ASSERT_EQ(outputs.size(), 1);

  const auto& table = inputs[0];     // Table tensor [vocab_size, hidden_size]
  const auto& indices = inputs[1];   // Indices tensor [B, S]
  auto& output = outputs[0];         // Output tensor [B, S, hidden_size]

  // Create aclTensors
  aclTensor* acl_weight = createAclTensor(table);
