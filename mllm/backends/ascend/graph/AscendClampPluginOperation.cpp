...
struct ClampCacheState {
  aclTensor* src_tensor{nullptr};
  aclTensor* dst_tensor{nullptr};
  aclScalar* min_scalar{nullptr};
  aclScalar* max_scalar{nullptr};
...
void destroyCacheState(ClampCacheState*& cache) {
  if (cache == nullptr) return;
  if (cache->executor != nullptr) {
    aclDestroyAclOpExecutor(cache->executor);
    cache->executor = nullptr;
  }
  if (cache->min_scalar != nullptr) {
    aclDestroyScalar(cache->min_scalar);
    cache->min_scalar = nullptr;
  }
  if (cache->max_scalar != nullptr) {
    aclDestroyScalar(cache->max_scalar);
    cache->max_scalar = nullptr;
  }
...
atb::Status updateTensorAddrs(ClampCacheState* cache, const atb::VariantPack& variantPack) {
...
AscendClampPluginOperation::AscendClampPluginOperation(float min_val, float max_val)
    : min_val_(min_val), max_val_(max_val) {}

AscendClampPluginOperation::~AscendClampPluginOperation() {
  auto* cache = static_cast<ClampCacheState*>(cache_state_);
  destroyCacheState(cache);
  cache_state_ = nullptr;
}

std::string AscendClampPluginOperation::GetName() const {
  return "AscendClampPluginOperation";
}

atb::Status AscendClampPluginOperation::InferShape(
...
uint32_t AscendClampPluginOperation::GetInputNum() const { return INPUT_NUM; }
uint32_t AscendClampPluginOperation::GetOutputNum() const { return OUTPUT_NUM; }

atb::Status AscendClampPluginOperation::Setup(const atb::VariantPack& variantPack,
  ...
  auto* new_cache = new ClampCacheState();
  ...
  new_cache->min_scalar = aclCreateScalar(&min_val_, ACL_FLOAT);
  new_cache->max_scalar = aclCreateScalar(&max_val_, ACL_FLOAT);
  if (new_cache->src_tensor == nullptr || new_cache->dst_tensor == nullptr ||
      new_cache->min_scalar == nullptr || new_cache->max_scalar == nullptr) {
    destroyCacheState(new_cache);
    return atb::ERROR_INTERNAL_ERROR;
  }

  aclnnStatus ret = aclnnClampGetWorkspaceSize(new_cache->src_tensor,
                                               new_cache->min_scalar,
                                               new_cache->max_scalar,
...
atb::Status AscendClampPluginOperation::Execute(const atb::VariantPack& variantPack,
                                                uint8_t* workspace,
                                                uint64_t workspace_size,
                                                atb::Context* context) {
  (void)context;
  ...
  aclError ret = aclnnClamp(workspace,
...
atb::Operation* createClampPluginGraphOp(float min_val, float max_val) {
  return new AscendClampPluginOperation(min_val, max_val);
}
