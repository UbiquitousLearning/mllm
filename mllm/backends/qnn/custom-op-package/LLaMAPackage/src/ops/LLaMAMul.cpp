...
GraphStatus llamamulImpl(TensorType& out_0, const TensorType& in_0, const TensorType& in_1);
 ...
 * e.g. DEF_PACKAGE_OP((llamamulImpl<Tensor>), "LLaMAMul")
 */
 ...
 * e.g. DEF_PACKAGE_OP_AND_COST_AND_FLAGS((llamamulImpl<PlainFloatTensor>), "LLaMAMul", SNAIL)
 */
...
int32_t hvx_mul_af(float* restrict input, float* restrict input2, float* restrict output, uint32_t size) {
  ...
  if (leftover_size > 0) {
    ...
    vstu_variable(optr, leftover_size, Q6_Vsf_equals_Vqf32(Q6_Vqf32_vmpy_VsfVsf(sline1, sline2)));
  }

  return 0;
}

int32_t hvx_mul_ahf(__fp16* restrict input, __fp16* restrict input2, __fp16* restrict output, uint32_t size) {
  if ((input == NULL) || (output == NULL) || (size == 0)) { return -1; }
