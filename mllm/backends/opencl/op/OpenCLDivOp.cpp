...
OpenCLDivOp::OpenCLDivOp(Backend *bn, std::string name, float data) :
    ...
    cl_int err;
    kernel_fp32_buffer_ = clCreateKernel(program, "div_scalar_float", &err);
    check_cl_error(err, "clCreateKernel for div_scalar_float");
    kernel_fp32_image_ = clCreateKernel(program, "div_scalar_float_image2d", &err);
    check_cl_error(err, "clCreateKernel for div_scalar_float_image2d");
    kernel_fp16_buffer_ = clCreateKernel(program, "div_scalar_fp16_vector", &err);
    check_cl_error(err, "clCreateKernel for div_scalar_fp16_vector");
    kernel_fp16_image_ = clCreateKernel(program, "div_scalar_fp16_image2d", &err);
    check_cl_error(err, "clCreateKernel for div_scalar_fp16_image2d");
...
ErrorCode OpenCLDivOp::reshape(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
...
ErrorCode OpenCLDivOp::setUp(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
...
ErrorCode OpenCLDivOp::execute(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
