 ...
 * CLK_NORMALIZED_COORDS_FALSE: Use non-normalized integer coordinates (pixel coordinates) instead of floating point coordinates in [0.0, 1.0].
 * CLK_ADDRESS_CLAMP_TO_EDGE: When reading coordinates beyond the image boundary, automatically return the pixel value of the nearest edge to prevent out-of-bounds reading.
 * CLK_FILTER_NEAREST: Read the pixel closest to the coordinate without interpolation, which is essential for data calculation.
 */
