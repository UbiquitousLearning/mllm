...
namespace mllm {
CPUVisionRoPESin::CPUVisionRoPESin(Backend *bn, string opName, int dim, int spatial_merge_size, int threadCount) :
...
// 计算并填充inv_freq向量的方法
void CPUVisionRoPESin::compute_inv_freq(int dim) {
...
std::vector<std::vector<float>> CPUVisionRoPESin::rotary_pos_emb_forward(int seqlen) {
    if (seqlen <= 0) {
...
#pragma omp parallel for num_threads(4) schedule(dynamic)
    for (size_t i = 0; i < seq.size(); ++i) {
        ...
        for (size_t j = 0; j < inv_freq.size(); ++j) {
            row[j] = std::sin(seq_val * inv_freq[j]);
        }
    }
    return freqs;
}

vector<vector<int>> CPUVisionRoPESin::rot_pos_emb(vector<vector<float>> grid_thw, int spatial_merge_size) {
...
void CPUVisionRoPESin::compute_rotary_pos_embd(
...
ErrorCode CPUVisionRoPESin::reshape(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
...
ErrorCode CPUVisionRoPESin::execute(vector<shared_ptr<Tensor>> inputs, vector<shared_ptr<Tensor>> outputs) {
