...
void CPUMeanOp::forward(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs) {
  ...
  if (dim == 0x7fffffff) {
    switch (dtype) {
      case kFloat32: {
#if defined(MLLM_HOST_ARCH_ARM64) || defined(MLLM_HOST_ARCH_ARM)
        arm::reduce_mean_fp32(output.ptr<mllm_fp32_t>(), input.ptr<mllm_fp32_t>(), input.numel(), options_.getThreads());
#endif
        break;
      }
      case kFloat16: {
#if defined(MLLM_HOST_ARCH_ARM64) || defined(MLLM_HOST_ARCH_ARM)
        arm::reduce_mean_fp16(output.ptr<mllm_fp16_t>(), input.ptr<mllm_fp16_t>(), input.numel(), options_.getThreads());
#endif
        break;
      }
      case kInt8: {
#if defined(MLLM_HOST_ARCH_ARM64) || defined(MLLM_HOST_ARCH_ARM)
        arm::reduce_mean_int8(output.ptr<mllm_int8_t>(), input.ptr<mllm_int8_t>(), input.numel(), options_.getThreads());
#endif
        break;
      }
      case kInt16: {
#if defined(MLLM_HOST_ARCH_ARM64) || defined(MLLM_HOST_ARCH_ARM)
        arm::reduce_mean_int16(output.ptr<mllm_int16_t>(), input.ptr<mllm_int16_t>(), input.numel(), options_.getThreads());
#endif
        break;
      }
      case kInt32: {
#if defined(MLLM_HOST_ARCH_ARM64) || defined(MLLM_HOST_ARCH_ARM)
        arm::reduce_mean_int32(output.ptr<mllm_int32_t>(), input.ptr<mllm_int32_t>(), input.numel(), options_.getThreads());
#endif
        break;
      }
  ...
  switch (dtype) {
    case kFloat32: {
      ...
      for (int out = 0; out < outer_size; ++out) {
        for (int in = 0; in < inner_size; ++in) {
#if defined(MLLM_HOST_ARCH_ARM64) || defined(MLLM_HOST_ARCH_ARM)
          arm::reduce_mean_fp32(&output_ptr[out * inner_size + in], &input_ptr[out * axis_size * inner_size + in], axis_size,
                                options_.getThreads());
#endif
        }
      }
    ...
    case kFloat16: {
      ...
      for (int out = 0; out < outer_size; ++out) {
        for (int in = 0; in < inner_size; ++in) {
#if defined(MLLM_HOST_ARCH_ARM64) || defined(MLLM_HOST_ARCH_ARM)
          arm::reduce_mean_fp16(&output_ptr[out * inner_size + in], &input_ptr[out * axis_size * inner_size + in], axis_size,
                                options_.getThreads());
#endif
        }
      }
    ...
    case kInt8: {
      ...
      for (int out = 0; out < outer_size; ++out) {
        for (int in = 0; in < inner_size; ++in) {
#if defined(MLLM_HOST_ARCH_ARM64) || defined(MLLM_HOST_ARCH_ARM)
          arm::reduce_mean_int8(&output_ptr[out * inner_size + in], &input_ptr[out * axis_size * inner_size + in], axis_size,
                                options_.getThreads());
#endif
        }
      }
    ...
    case kInt16: {
      ...
      for (int out = 0; out < outer_size; ++out) {
        for (int in = 0; in < inner_size; ++in) {
#if defined(MLLM_HOST_ARCH_ARM64) || defined(MLLM_HOST_ARCH_ARM)
          arm::reduce_mean_int16(&output_ptr[out * inner_size + in], &input_ptr[out * axis_size * inner_size + in], axis_size,
                                 options_.getThreads());
#endif
        }
      }
    ...
    case kInt32: {
      ...
      for (int out = 0; out < outer_size; ++out) {
        for (int in = 0; in < inner_size; ++in) {
#if defined(MLLM_HOST_ARCH_ARM64) || defined(MLLM_HOST_ARCH_ARM)
          arm::reduce_mean_int32(&output_ptr[out * inner_size + in], &input_ptr[out * axis_size * inner_size + in], axis_size,
                                 options_.getThreads());
#endif
        }
      }
