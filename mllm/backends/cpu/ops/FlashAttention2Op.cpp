...
void CPUFlashAttention2Op::forward(const std::vector<Tensor>& inputs, std::vector<Tensor>& outputs) {
  ...
  switch (Q.dtype()) {
    case kFloat32: {
      ...
      cpu::flash_attn2::fwd_bhsd<cpu::flash_attn2::details::__ArmArchTag, mllm_fp32_t, mllm_fp32_t, mllm_fp32_t, mllm_fp32_t,
      ...
      cpu::flash_attn2::fwd_bhsd<cpu::flash_attn2::details::__X86ArchTag, mllm_fp32_t, mllm_fp32_t, mllm_fp32_t, mllm_fp32_t,
                                 mllm_fp32_t>(B, options_.q_head, options_.kv_head, S_Q, S_KV, D, Q.ptr<mllm_fp32_t>(),
                                              K.ptr<mllm_fp32_t>(), V.ptr<mllm_fp32_t>(), O.ptr<mllm_fp32_t>(),
                                              options_.getThreads());
#endif
