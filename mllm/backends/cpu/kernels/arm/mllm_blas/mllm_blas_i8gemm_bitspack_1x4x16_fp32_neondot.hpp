    ...
    for (int n_idx = 0; n_idx < n; n_idx += 4) {
      ...
      for (int k_idx = 0; k_idx < k; k_idx += group_size) {
        ...
        for (int i = 0; i < group_size; i += 16) {
          ...
          bitspack::vec_unpack_64_lowbit_values<weight_nbit>(weight_q_cols01_0, weight_q_cols23_0, weight_q_cols01_1,
                                                             weight_q_cols23_1, (uint8_t*)weight_data_byte_ptr);
        ...
        float32x4_t scale_factor = vmulq_f32(weight_scales, vdupq_n_f32(activation_scale));
        ...
        if (has_weight_zeros) {
          ...
          int32x4_t term3 = vmulq_n_s32(weight_zeros, group_size * activation_zero);
      ...
      if (has_clamp) { res = internal::clamp(res, clamp_min, clamp_max); }
