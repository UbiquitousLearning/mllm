    ...
    for (int n_idx = 0; n_idx < n; n_idx++) {
      ...
      for (int k_idx = 0; k_idx < k; k_idx += group_size) {
        ...
        for (int i = 0; i < group_size; i += 32) {
          bitspack::vec_unpack_32_lowbit_values<weight_nbit>(
              /*unpacked0=*/wq0,
              /*unpacked1=*/wq1,
        ...
        if (has_weight_zeros) {
          ...
          res += (weight_scale * activation_scale)
                 * (qval_dot - (activation_zero * weight_qvals_sum) - (weight_zero * activation_qvals_sum)
                    + (group_size * weight_zero * activation_zero));
        } else {
          res += (weight_scale * activation_scale) * (qval_dot - activation_zero * weight_qvals_sum);
        }
      }  // k_idx
      if (has_bias) {
