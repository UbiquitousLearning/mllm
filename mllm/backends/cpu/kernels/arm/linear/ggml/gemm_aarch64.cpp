...
void quantize_mat_q8_0(const float* __restrict x, void* __restrict vy, int64_t nrow, int64_t n_per_row,
...
static size_t quantize_q4_0_nr_bl(const float* __restrict src, void* __restrict dst, int64_t nrow, int64_t n_per_row,
                                  int nrows_interleaved, int blck_size_interleave) {
...
// #elif defined(__ARM_NEON) && defined(__aarch64__)
  ...
  __asm__ __volatile__("movi v31.16b, #0x4\n"
                       ...
                       : "memory", "v16", "v17", "v18", "v19", "v20", "v21", "v22", "v23", "v24", "v25", "v26", "v27", "v28",
                         "v29", "v30", "v31", "x20", "x21", "x22");
#else
  float sumf[4];
  int sumi;
...
// #elif defined(__ARM_NEON) && defined(__aarch64__)
  ...
  for (int x = 0; x < nc / ncols_interleaved; x++) {
    ...
    for (int j = 0; j < ncols_interleaved; j++) { sumf[j] = bias_ptr[x * ncols_interleaved + j]; }
...
// #endif
  ...
  __asm__ __volatile__("movi v2.16b, #0x4\n"
                       ...
                       : [b_ptr] "+&r"(b_ptr), [res_ptr] "+&r"(res_ptr), [nc] "+&r"(nc)
                       : [a_ptr] "r"(a_ptr), [nb] "r"(nb)
                       : "memory", "v0", "v1", "v2", "v16", "v17", "v18", "v19", "v20", "v21", "v22", "v23", "v24", "v25",
                         "v26", "v27", "v28", "v29", "v30", "v31", "x20", "x21", "x22", "x23");
#elif defined(__ARM_NEON) && defined(__aarch64__)
...
void mllm_gemv_q4_0_8x8_q8_0(int n, float* __restrict s, size_t bs, const void* __restrict vx, const void* __restrict vy,
  ...
  if (svcntw() == 8) {
    ...
    __asm__ __volatile__("ptrue p0.b\n"
                         ...
                         : [b_ptr] "+&r"(b_ptr), [res_ptr] "+&r"(res_ptr), [nc] "+&r"(nc)
                         : [a_ptr] "r"(a_ptr), [nb] "r"(nb)
                         : "memory", "p0", "x20", "x21", "x22", "z16", "z17", "z18", "z19", "z20", "z21", "z22", "z23", "z24",
...
#if defined(__ARM_NEON)
    MLLM_WARN_EXIT(ExitCode::kAssert, "_mllm_gemm_q4_0_4x4_q8_0_bias not implemented");
#endif
    return;
  }
...
// #endif
  ...
  __asm__ __volatile__("mov x10, %x[nr]\n"
                       ...
                       : "cc", "memory", "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9", "v10", "v11", "v12", "v13",
                         ...
                         "v28", "v29", "v30", "v31", "x9", "x10", "x20", "x21", "x22", "x23", "x24", "x25", "x26", "x27",
                         "x28");
#else
  float sumf[4][4];
  int sumi;
  ...
  for (int y = 0; y < nr / 4; y++) {
    ...
    for (int x = 0; x < nc / ncols_interleaved; x++) {
      ...
      for (int m = 0; m < 4; m++) {
        for (int j = 0; j < ncols_interleaved; j++) s[(y * 4 + m) * bs + x * ncols_interleaved + j] = sumf[m][j];
      }
    }
  }
#endif
...
void mllm_gemm_q4_0_4x8_q8_0(int n, float* __restrict s, size_t bs, const void* __restrict vx, const void* __restrict vy,
  ...
  if (bias != nullptr) {
#if defined(__ARM_NEON)
    MLLM_WARN_EXIT(ExitCode::kAssert, "_mllm_gemm_q4_0_4x8_q8_0_bias not implemented");
#endif
...
void mllm_gemm_q4_0_8x8_q8_0(int n, float* __restrict s, size_t bs, const void* __restrict vx, const void* __restrict vy,
  ...
  if (bias != nullptr) {
#if defined(__ARM_NEON)
    MLLM_WARN_EXIT(ExitCode::kAssert, "_mllm_gemm_q4_0_8x8_q8_0_bias not implemented");
#endif
  ...
  if (true) {
    ...
    __asm__ __volatile__("mov x20, #0x4\n"
                         ...
                         : [a_ptr] "+&r"(a_ptr), [res_ptr] "+&r"(res_ptr)
                         : [b_ptr] "r"(b_ptr), [nr] "r"(nr), [nb] "r"(nb), [res_stride] "r"(res_stride), [nc] "r"(nc)
                         : "cc", "memory", "p0", "p1", "x9", "x10", "x11", "x12", "x13", "x20", "x21", "x22", "x23", "x24",
                           "x25", "x26", "x27", "x28", "z0", "z1", "z2", "z3", "z4", "z5", "z6", "z7", "z8", "z9", "z10", "z11",
                           "z12", "z13", "z14", "z15", "z16", "z17", "z18", "z19", "z20", "z21", "z22", "z23", "z24", "z25",
