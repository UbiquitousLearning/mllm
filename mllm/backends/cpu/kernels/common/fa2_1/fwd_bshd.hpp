...
namespace mllm::cpu::flash_attn2 {
    ...
    MLLM_CONDITIONAL_PARALLEL_FOR(thread_count > 1, thread_count, h_q_idx, 0, H_Q, 1, {
      ...
      for (int s_q_idx = 0; s_q_idx < S_Q; ++s_q_idx) {
        ...
        for (int s_kv_idx = 0; s_kv_idx < S_KV_BOUND; ++s_kv_idx) {
          ...
          __KDType* k_token = __k + b_idx * S_KV * H_KV * D + s_kv_idx * H_KV * D;
          __VDType* v_token = __v + b_idx * S_KV * H_KV * D + s_kv_idx * H_KV * D;

          // Offset to one head.
          // k_token and v_token shape is [D]
          k_token = k_token + h_kv_id * D;
          v_token = v_token + h_kv_id * D;
