// Auto generated: 2026-09-06 09:09:14
...
#define RTTI_RK_OP_IMPL(v) \
    return (v)->getKind() >= RK_Op && (v)->getKind() <= RK_Op_Last \
...
#define RTTI_RK_OP_LINALGIROP_CAUSALDEPTHWISECONV1DOP_IMPL(v) \
    return (v)->getKind() >= RK_Op_LinalgIROp_CausalDepthwiseConv1DOp && (v)->getKind() <= RK_Op_LinalgIROp_CausalDepthwiseConv1DOp \

#define RTTI_RK_OP_LINALGIROP_GROUPEDQUERYATTENTIONOP_IMPL(v) \
    return (v)->getKind() >= RK_Op_LinalgIROp_GroupedQueryAttentionOp && (v)->getKind() <= RK_Op_LinalgIROp_GroupedQueryAttentionOp \
...
#define RTTI_RK_OP_LINALGIROP_FLASHATTENTION2SWASINKOP_IMPL(v) \
    return (v)->getKind() >= RK_Op_LinalgIROp_FlashAttention2SwaSinkOp && (v)->getKind() <= RK_Op_LinalgIROp_FlashAttention2SwaSinkOp \
...
#define RTTI_RK_OP_DBGIROP_IMPL(v) \
    return (v)->getKind() >= RK_Op_DbgIROp && (v)->getKind() <= RK_Op_DbgIROp_Last \
...
#define RTTI_RK_VAL_IMPL(v) \
    return (v)->getKind() >= RK_Val && (v)->getKind() <= RK_Val_Last \
...
#define RTTI_RK_VAL_DBGIRVAL_IMPL(v) \
    return (v)->getKind() >= RK_Val_DbgIRVal && (v)->getKind() <= RK_Val_DbgIRVal \

#define RTTI_RK_ATTR_IMPL(v) \
    return (v)->getKind() >= RK_Attr && (v)->getKind() <= RK_Attr_Last \
...
#define RTTI_RK_ATTR_LINALGIRATTR_QUANTIZATIONANNOTATION_IMPL(v) \
    return (v)->getKind() >= RK_Attr_LinalgIRAttr_QuantizationAnnotation && (v)->getKind() <= RK_Attr_LinalgIRAttr_QuantizationAnnotation \
...
#define RTTI_RK_ATTR_DBGIRATTR_IMPL(v) \
    return (v)->getKind() >= RK_Attr_DbgIRAttr && (v)->getKind() <= RK_Attr_DbgIRAttr \
